...
use App\Notifications\DriverOnWayDoneOrderNotification;
...
class DriverOnWayDoneOrderListener
{
    public function handle(DriverOnWayDoneOrderEvent $event)
    {
        $order = $event->order;
...
//            if ($branch) Notification::send($branch, new DriverOnWayOrderNotification($order));
            if ($user) Notification::send($user, new DriverOnWayDoneOrderNotification($order));
            $order->update([
                'status' => Order::ON_WAY_DONE,
